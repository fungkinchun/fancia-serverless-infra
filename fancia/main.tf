...
locals {
  ...
  jdbc_database_name = coalesce(
    ...
    local.is_prod ? var.project_name : "${var.project_name}_${var.environment}"
  )
  ...
  cors_allowed_origins = distinct(compact([
    ...
    "https://www.${local.dns_name}",
    "https://${local.apex_domain}",
    "https://www.${local.apex_domain}",
    "https://dev.${local.apex_domain}",
    "http://localhost:3000",
  ]))
}
...
resource "aws_lb_listener_rule" "internal" {
  ...
  condition {
    host_header {
      values = ["internal.${local.dns_name}"]
    }
  }
}
...
module "api_lambda" {
  ...
  ecr_repository_name               = "${var.project_name}-${var.environment}-backend-${each.value.name}"
  domain_name                       = local.dns_name
  internal_dns_domain               = local.internal_dns
  rds_dns_domain                    = local.rds_dns
  repo_name                         = each.value.name
  database_name                     = each.value.database_name
  database_secret_name              = each.value.database_secret_name
  jdbc_database_name                = coalesce(try(each.value.jdbc_database_name, null), local.jdbc_database_name)
