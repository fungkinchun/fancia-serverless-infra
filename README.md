...
- **SnapStart and Docker images.** SnapStart does **not** support Lambda container images. It only works with ZIP deployments on supported managed runtimes (Java 11+, Python 3.12+, .NET 8+). Prefer the S3/ZIP package path when `enable_snapstart` is on; for ECR/image-based functions use provisioned concurrency (or accept normal cold starts) instead.
